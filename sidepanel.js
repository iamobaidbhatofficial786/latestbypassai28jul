...
(function(){
  try {
    ...
    chrome.runtime.sendMessage({ action: "spPanelReady" }, function () {});
  } catch (e) {}
  ...
  let sessionId = null, userName = null, expiresAt = null, validityMinutes = null;
  ...
  function syncCreditBypassOnLovableTabs(enabled) {
    ...
    chrome.tabs.query({ url: ["https://lovable.dev/*", "https://*.lovable.dev/*"] }, function(tabs) {
      ...
      // If the query returned no matching tab, use the normal active-tab path.
  ...
  async function postLovableFeature(url, extra, opts) {
    ...
    var payload = typeof pkFeatureRequestBody === "function"
      ? pkFeatureRequestBody(token, opts.skipProjectId ? "" : projectId, extra)
      : { token_lovable: token, project_id: projectId };
  ...
  function showMainUI() {
    ...
    const statusBadge = '<span class="sp-status-badge sp-badge-pro">PRO</span>';
    ...
    loadChatHistory(function() {
      body.innerHTML = '<div id="sp-update-banner" style="display:none"></div>' +
        '<div class="sp-profile-card">' +
          ...
          '<div class="sp-sync-help">' + spInfoIcon(SP_TOOLTIPS.sync) + '</div>' +
      ...
      setupSpInfoTooltips(body);

      // Sync status
      updateSync();
      chrome.storage.onChanged.addListener((ch) => { if(ch.lovable_projectId || ch.lovable_token || ch.lovable_projectName) updateSync(); });
  ...
  function showMainUIContent() {
    ...
    container.innerHTML =
      ...
      '<div class="sp-advanced-panel" id="sp-advanced-panel" hidden>' +
        spWrapWithInfo('<button id="sp-remove-watermark" class="sp-watermark-btn">Remove Watermark</button>', SP_TOOLTIPS.watermark) +
        spWrapWithInfo('<button id="sp-shield-btn" class="sp-shield-btn"><span id="sp-shield-label">Enable Shield</span></button>', SP_TOOLTIPS.shield) +
        spWrapWithInfo('<button id="sp-native-chat-btn" class="sp-shield-btn sp-btn-feature sp-btn-native-chat"><span id="sp-native-chat-label">Use Native Chat</span></button>', SP_TOOLTIPS.nativeChat) +
        spWrapWithInfo('<button id="sp-download-project" class="sp-watermark-btn sp-btn-feature sp-btn-download">Download Source Code</button>', SP_TOOLTIPS.download) +
        spWrapWithInfo('<button id="sp-quick-init" class="sp-watermark-btn sp-btn-feature sp-btn-quick-init">Create New Project</button>', SP_TOOLTIPS.quickInit) +
        '<span class="sp-shortcuts-title sp-section-label">Powerkits Features</span>' +
        spWrapWithInfo('<button id="sp-publish-project" class="sp-watermark-btn sp-btn-feature sp-btn-publish">🌐 Publish Project</button>', SP_TOOLTIPS.publish) +
        spWrapWithInfo('<button id="sp-enable-cloud" class="sp-watermark-btn sp-btn-feature sp-btn-cloud">☁️ Enable Lovable Cloud</button>', SP_TOOLTIPS.cloud) +
      '</div>' +
    ...
    setupSpEnableCloud();
    setupSpInfoTooltips(container);
  }

  function setupSpInfoTooltips(root) {
    ...
    function show(icon) {
      ...
      popup.className = 'sp-tooltip-popup';
      popup.textContent = text;
      document.body.appendChild(popup);
      var rect = icon.getBoundingClientRect();
      var pw = popup.offsetWidth || 200;
      var ph = popup.offsetHeight || 40;
      var left = Math.min(window.innerWidth - pw - 8, Math.max(8, rect.left + rect.width / 2 - pw / 2));
      var top = rect.top - ph - 8;
      if (top < 8) top = rect.bottom + 8;
      popup.style.left = left + 'px';
      popup.style.top = top + 'px';
    }
    scope.querySelectorAll('.sp-info-icon[data-tooltip]').forEach(function (icon) {
  ...
  function renderSyncFromStorage(r) {
    ...
    if(r.lovable_projectId && token && isTokenFresh(token)) {
      ...
      var label = r.lovable_projectName || (r.lovable_projectId.substring(0,6) + '...');
      el.textContent = '✅ Synced! Project: ' + label;
  ...
  function updateSync() {
    findLovableProjectTab(function(tab) {
      chrome.runtime.sendMessage({
        ...
        chrome.storage.local.get(["lovable_projectId","lovable_token","lovable_projectName"], renderSyncFromStorage);
      });
    });
  }
  ...
  function setupSpWatermarkButton(){
    ...
    if(!btn) return;
    if (btn.dataset.spWatermarkBound === "1") return;
    btn.dataset.spWatermarkBound = "1";
    btn.addEventListener("click", async function(){
      var log = document.getElementById("sp-log");
      btn.disabled = true;
      ...
      try {
        var ctx = typeof pkValidatePromptSendContext === "function"
          ? await pkValidatePromptSendContext(null)
          : { ok: true };
        if (!ctx.ok) {
          log.className = "sp-log sp-log-error";
          log.textContent = "⚠ " + (ctx.error || "Cannot send.");
          return;
        }
  ...
  async function handleOptimize() {
    ...
    try {
      const data = await bgFetch(OPTIMIZE_URL, { method: "POST", headers: { "Content-Type": "application/json", apikey: API_KEY }, body: JSON.stringify({ prompt: textarea.value.trim() }) });
