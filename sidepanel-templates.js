...
/** Vendor 3.8.9-aligned feature help copy (English). */
const SP_TOOLTIPS = {
  watermark: 'Removes Lovable branding/watermark from your published project.',
  shield: 'Protects your project UI from casual inspection while you work.',
  nativeChat: 'Hides the extension UI and lets you type directly in Lovable chat with credit bypass still active.',
  download: 'Downloads the current project source code as a ZIP from Lovable.',
  quickInit: 'On the Lovable home screen, creates an empty project (placeholder + Build), then stop — send your real prompt from the extension.',
  sync: 'Shows sync status with your open Lovable project tab and session.',
  publish: 'Publishes the current project through Powerkits feature relay.',
  cloud: 'Enables Lovable Cloud for the current project through Powerkits feature relay.'
};
...
function spInfoIcon(tooltipText) {
  return '<span class="sp-info-icon" data-tooltip="' + spEscapeHtml(tooltipText) + '" tabindex="0" role="img" aria-label="Info">' + SP_SVG.info + '</span>';
}

function spWrapWithInfo(buttonHtml, tooltipText) {
  return '<div class="sp-btn-with-info">' + buttonHtml + spInfoIcon(tooltipText) + '</div>';
}
